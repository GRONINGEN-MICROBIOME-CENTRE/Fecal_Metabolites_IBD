...
my_cc_categories=my_cc_quantitative_resec
my_cc_categories$SUPER.PATHWAY[is.na(my_cc_categories$SUPER.PATHWAY)]="SCFA"
my_cc_categories$SUB.PATHWAY[is.na(my_cc_categories$SUB.PATHWAY)]="SCFA"
...
enrich_two_compare_resec = foreach(i=1:nrow(sub_pathway_count),.combine = rbind) %do%  { # compare t statistics between CD and UC
...
enrich_two_compare_resec$FDR=p.adjust(enrich_two_compare_resec$Pvalue,method="BH")
